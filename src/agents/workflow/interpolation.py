"""Prompt template interpolation for workflow steps.

Resolves {{ steps.X.output.Y }} placeholders from workflow state.
Values are wrapped in <data>...</data> delimiters to help LLMs
distinguish injected data from instructions.
"""

from __future__ import annotations

import json
import re

from agents.workflow.state import WorkflowState

TEMPLATE_PATTERN = re.compile(r"\{\{\s*steps\.(\w+)\.output\.(\w+)\s*\}\}")


def interpolate(template: str, state: WorkflowState) -> str:
    """Replace {{ steps.X.output.Y }} with values from workflow state.

    Rules:
    - Missing step or key → raises ValueError (fail fast)
    - str values → wrapped in <data>...</data>
    - dict/list values → JSON-serialized, wrapped in <data>...</data>
    - None → <data>null</data>

    Args:
        template: Prompt template with {{ }} placeholders.
        state: Current workflow state with step results.

    Returns:
        Interpolated prompt string.

    Raises:
        ValueError: If a referenced step or output key is missing.
    """

    def replacer(match: re.Match) -> str:
        step_name, key = match.group(1), match.group(2)
        result = state.steps.get(step_name)
        if result is None or result.output is None:
            raise ValueError(
                f"Template references missing step or output: "
                f"steps.{step_name}.output.{key}"
            )
        value = result.output.get(key)
        if value is None:
            return "<data>null</data>"
        if isinstance(value, bool):
            return f"<data>{json.dumps(value)}</data>"
        if isinstance(value, (dict, list)):
            return f"<data>{json.dumps(value)}</data>"
        return f"<data>{value}</data>"

    return TEMPLATE_PATTERN.sub(replacer, template)
