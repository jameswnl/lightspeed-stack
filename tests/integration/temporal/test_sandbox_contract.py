...
class TestSandboxRequestContract:
    ...
    async def test_request_body_matches_contract(
        ...
        await run_sandbox_step({
            "step": {
                "name": "diag", "prompt": "check cluster health",
                "output_key": "r1", "instructions": "You are a K8s expert",
                "output_schema": {"type": "object"},
            },
            "workflow_id": "wf-contract-1",
            "provider": {"name": "openai", "model": "gpt-4", "credentials_secret": "key"},
...
class TestSandboxResponseContract:
    ...
    async def test_success_true_returns_completed(
        ...
        mock_http = mocker.patch("agents.workflow.temporal_activities.httpx.AsyncClient")
